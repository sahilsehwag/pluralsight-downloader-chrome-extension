...
export const mapCourses = (fn: Fn1<Course[], Course[]>) => pipe(
  getCourses,
  T.map(fn),
  T.flatMap(setCourses),
)

export const mapCourse = courseId => (fn: Fn1<Course, Course>) =>
  mapCourses(
    A.map(
      course => course.id === courseId ? fn(course) : course
    )
  )

// TODO: instead of Course[] should be map of courseId and course
export const addCourse = (course: Course) => mapCourses(
  courses => pipe(
    courses,
