...
import {
	fetchExcerciseFilesURL,
	fetchVideoURL,
	getSubtitleURL,
} from '~/configs/platform/pluralsight/service'

const getPlaylistFileContent = ({ course }: { course: Course }) =>
	pipe(
...
const getDownloadItemsFromVideo = ({ course, sectionIdx, videoIdx, video }) =>
	pipe(
		TE.Do,
		...
		TE.map(({ vUrl, sUrl }) => [
			...
			buildDownloadItem({
				...
				videoIdx,
				type: 'SUBTITLE',
				pathBuilder: buildSubtitlePath,
			}),
		]),
	)

// FIX: one failed url fetch will cause entire thing not to run
type GetDownloadItemsFromSectionParams = {
	course: Course
	sectionIdx: number
	section?: Section
}
...
}: GetDownloadItemsFromSectionParams) =>
	pipe(
		SE.getVideos(section),
		A.mapWithIndex((videoIdx, video) =>
...
const getDownloadItemsFromCourse = (course: Course) =>
	pipe(
		CE.getSections(course),
...
export const downloadPlaylistFile = (course: Course) =>
	pipe(
		TE.Do,
...
export const downloadExcerciseFiles = (course: Course) =>
	pipe(
		TE.Do,
...
export const addSectionToQueue = (course: Course, sectionIdx: number) =>
	pipe(
		getDownloadItemsFromSection({ course, sectionIdx }),
		TE.flatMapTask(addToQueue),
		TE.mapError(tap(logError)),
	)

export const addCourseToQueue = (course: Course) =>
	pipe(
		getDownloadItemsFromCourse(course),
		TE.flatMapTask(addToQueue),
		TE.mapError(tap(logError)),
	)

export const downloadCourse = (course: Course) =>
	pipe(
